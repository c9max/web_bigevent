$(function() {
    ...
    function renderPage(total) {
        ...
        laypage.render({
            ...
            jump: function(obj, first) {
                ...
                q.pagenum = obj.curr;
                //把最新的每页条目数，赋值给q这个查询参数对象中
                q.pagesize = obj.limit;
                //这是根据最新的q获取对应的数据列表，并渲染表格
                // initTable();
                //可以通过first值，来判断是通过那种方式，触发的jump回调
                if (!first) {
                    //如果first不为true，证明是点击页码触发的jump回调
                    initTable();
                }
            }
        })
    ...
    $("tbody").on("click", ".btn-edit", function() {
        var id = $(this).attr("data-id");
        //询问用户是否要删除数据
        layer.confirm('确认前往编辑?', { icon: 3, title: '提示' }, function(index) {
